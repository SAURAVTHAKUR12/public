...
for j=1:channels
    for i=1:size1
        ...
        for m=-10:10
            for n=-10:10
                if i+m>0 && k+n>0 && i+m<= size1 && k+n<= size2
                    l = (J(i+m,k+n,j)-J(i,k,j))^2;
                    d =  m^2+n^2;              
                    w_sum = w_sum + (exp(-1*l/sigr)*exp(-1*d/sigd))*J(i+m,k+n,j);
                 w_total = w_total + (exp(-1*l/sigr)*exp(-1*d/sigd));
                end
             end
         end

         N(i,k,j) = double(w_sum/w_total);
        end
    end
end
G1 = rgb2gray(N);
        ...
        for m=-10:10
            for n=-10:10
                if i+m>0 && k+n>0 && i+m<= size1 && k+n<= size2
                    l = (J(i+m,k+n)-J(i,k))^2;
                    d =  m^2+n^2;              
                    w_sum = w_sum + (exp(-1*l/sigr)*exp(-1*d/sigd))*J(i+m,k+n);
                 w_total = w_total + (exp(-1*l/sigr)*exp(-1*d/sigd));
                end
             end
         end

         N(i,k) = double(w_sum/w_total);
             end
       end
    N = imadjust(N);
end
...
for l=1:size(BB,1)
    for i=BB(l,1):BB(l,1)+ BB(l,3)
     for j=BB(l,2): BB(l,2)+ BB(l,4)
         if I(j,i,1) > 20 && I(j,i,2)<100 && I(j,i,3)<100
             I(j,i,1) = 0;
         end
     end
    end
